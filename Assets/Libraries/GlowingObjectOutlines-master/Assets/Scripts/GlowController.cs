...
public class GlowController : MonoBehaviour
{
	...
	private void RebuildCommandBuffer()
	{
		...
		_commandBuffer.GetTemporaryRT(_prePassRenderTexID, Screen.width, Screen.height, 0, FilterMode.Point, RenderTextureFormat.ARGB32, RenderTextureReadWrite.Default, QualitySettings.antiAliasing);
		...
		for (int i = 0; i < _glowableObjects.Count; i++)
		{
			...
			var textures = _glowableObjects[i].Textures;
			for (int j = 0; j < _glowableObjects[i].Renderers.Length; j++)
			{
				_commandBuffer.SetGlobalTexture("_GlowImage", textures[j]);
		...
		_commandBuffer.GetTemporaryRT(_blurPassRenderTexID, Screen.width, Screen.height, 0, FilterMode.Point);
		_commandBuffer.GetTemporaryRT(_tempRenderTexID, Screen.width , Screen.height, 0, FilterMode.Point);
		_commandBuffer.Blit(_prePassRenderTexID, _blurPassRenderTexID);

		_blurTexelSize = new Vector2(1f / (Screen.width), 1f / (Screen.height));
