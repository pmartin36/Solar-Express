Shader "Sprites/ShipTrail2"
{
	...
	SubShader
	{
		...
		CGPROGRAM
			...
			{
				float lut_y = i.uv.y + 0.1 * (1-saturate(i.uv.x+0.0)) * sin((_Time.y + i.uv.x)*5);
				...
				float ycloud = (i.uv.y + _Time.x/9);
				fixed4 clouds = tex2D(_CloudTex, float2(i.uv.x + _Time.x*3, ycloud));

				r.a *=  max(0.7, pow(clouds.r + 0.6, 4)) * pow(1.1 - abs(yneg),1.5) + c.b/3;
				r *= c.a;
